...
def _classify_status_from_landmarks(landmarks):
    ...
    wrists_above_head = left_wrist.y < nose.y and right_wrist.y < nose.y

    wrists_close_to_chest = (
        left_wrist.y > left_shoulder.y and
        right_wrist.y > right_shoulder.y and
        abs(left_wrist.x - left_shoulder.x) < 0.25 and
        abs(right_wrist.x - right_shoulder.x) < 0.25
    )

    arms_crossed = (
        _distance(left_wrist, right_elbow) < 0.15 and
        _distance(right_wrist, left_elbow) < 0.15
    )
    ...
    if wrists_above_head:
        return "ok"
    if arms_crossed or wrists_close_to_chest:
        return "medical"
